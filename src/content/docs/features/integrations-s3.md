...
In v1 the toggle is read once at startup, so a server restart is required after flipping it. Until you restart, requests to `/s3/...` get a 404 — which is the whole point: a surface you don't use is a surface that isn't exposed.
...
- **No background sweeper**: orphan multipart staging dirs from clients that crash mid-upload survive a server restart and need manual cleanup (`rm -rf APP_DATA_DIR/.s3-multipart/`). A periodic sweeper with configurable TTL is planned for a later phase.
